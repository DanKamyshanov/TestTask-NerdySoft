...
import org.danylo.entity.Order;
import org.danylo.entity.User;
import org.danylo.entity.product.Product;
import org.danylo.entity.product.RealProduct;
import org.danylo.factory.ProductFactory;
import org.danylo.singleton.VirtualProductCodeManager;
...
public class Main {
    public static void main(String[] args) {
        ...
        virtualProductCodeManager.useCode("abc");

        System.out.println("Is code 'xxx' used: " + virtualProductCodeManager.isCodeUsed("xxx") + "\n");
        System.out.println("Is code 'yyy' used: " + virtualProductCodeManager.isCodeUsed("yyy") + "\n");
        System.out.println("Is code 'abc' used: " + virtualProductCodeManager.isCodeUsed("abc") + "\n");
    ...
    private static Product getMostPopularProduct(List<Order> orders) {
        return getProductUserMap(orders).entrySet().stream()
    ...
    private static Map<Product, List<User>> getProductUserMap(List<Order> orders) {
        return orders.stream()
                .flatMap(order -> order.getProducts().stream().map(product -> Map.entry(product, order.getUser())))
